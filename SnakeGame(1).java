...
public class SnakeGame(1) extends JPanel implements ActionListener, KeyListener {
  ...
  public void writeScorefile(ArrayList snakeBody){ // writes the file
    try (printWriter w = new PrintWriter(new FileWriter("D:\\user\\Records.txt"))){
    LinkedList<String> score = new LinkedList<String>();
    int thebest = 0 ;
    if (snakeBody.size())>thebest){
      score.add(snakeBody.size());
